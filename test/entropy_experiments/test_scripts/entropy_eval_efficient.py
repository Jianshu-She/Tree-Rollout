#!/usr/bin/env python3
"""
Efficient multi-sample entropy evaluation with optimized batching and reduced compute
"""
import torch
import numpy as np
import pandas as pd
...
class EfficientMultiSampleEvaluator:
    ...
    @torch.no_grad()
    def generate_batch_samples(self, prompts: List[str], samples_per_prompt: int = 8, max_new_tokens: int = 150) -> List[List[Tuple[str, List[float]]]]:
        """
        Generate multiple samples for multiple prompts efficiently using batching
        """
        all_results = []
        
        for prompt in prompts:
            # Prepare batch for this single prompt with multiple samples
            inputs = self.tokenizer(prompt, return_tensors="pt")
            inputs = {k: v.to(self.model.device) for k, v in inputs.items()}
            
            # Expand inputs to batch size
            batch_inputs = {k: v.repeat(samples_per_prompt, 1) for k, v in inputs.items()}
            ...
            for sample_idx in range(samples_per_prompt):
                ...
                # Extract entropy from scores (limit to first 200 tokens for efficiency)
                entropies = []
                if hasattr(outputs, 'scores') and outputs.scores:
                    max_entropy_tokens = min(200, len(outputs.scores))
                    for i in range(max_entropy_tokens):
                        entropy = self.calculate_entropy(outputs.scores[i][sample_idx])
                        entropies.append(entropy)
                
                prompt_results.append((generated_text, entropies))
...
Solution:"""
    ...
    def evaluate_dataset(self, problems: List[Dict], max_questions: int = 50, samples_per_question: int = 8) -> List[Dict]:
        """Evaluate model with multiple samples per question - optimized version"""
        results = []
        ...
        for batch_start in range(0, len(problems_subset), batch_size):
            batch_end = min(batch_start + batch_size, len(problems_subset))
            batch_problems = problems_subset[batch_start:batch_end]
            ...
            for problem in batch_problems:
                ...
                batch_data.append({
                    ...
                    'question_id': len(results) + len(batch_data)
                })
            
            logger.info(f"Processing batch {batch_start//batch_size + 1} ({len(prompts)} questions, {samples_per_question} samples each)")
            
            # Generate samples for all prompts in this batch
            batch_sample_results = self.generate_batch_samples(prompts, samples_per_question)
            
            # Process results for each question in the batch
            for q_idx, (prompt_samples, question_data) in enumerate(zip(batch_sample_results, batch_data)):
                question_samples = []
                correct_count = 0
                
                for sample_idx, (generated_text, entropies) in enumerate(prompt_samples):
                    # Extract predicted answer
                    predicted_answer = self.extract_answer(generated_text)
                    
                    # Check correctness
                    is_correct = self.check_answer_correctness(predicted_answer, question_data['expected_answer'])
                    if is_correct:
                        correct_count += 1
            ...
            logger.info(f"Processed {len(results)}/{max_questions} questions. Overall avg accuracy: {overall_accuracy:.1%}")
    ...
    def plot_results(self, results: List[Dict], analysis: Dict, output_dir: str = "."):
        ...
        fig.suptitle('Multi-Sample Entropy vs Question Accuracy Analysis (Efficient)', fontsize=16)
        ...
        for i, token_range in enumerate(token_ranges):
            ...
            # Row 2: Entropy variance vs accuracy
            ax = axes[1, i]
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/efficient_multi_sample_analysis.png", dpi=300, bbox_inches='tight')
        plt.show()


def main():
    parser = argparse.ArgumentParser(description="Efficient multi-sample entropy evaluation")
    parser.add_argument("--model", default="Qwen/Qwen2.5-7B-Instruct", help="Model name")
    parser.add_argument("--max_questions", type=int, default=25, help="Max questions to evaluate")
    parser.add_argument("--samples_per_question", type=int, default=8, help="Samples per question")
    parser.add_argument("--output_dir", default="./entropy_results_efficient", help="Output directory")
    ...
    evaluator = EfficientMultiSampleEvaluator(args.model)
    ...
    logger.info(f"Starting efficient multi-sample evaluation: {args.max_questions} questions × {args.samples_per_question} samples")
    ...
    if results:
        ...
        print(f"\n{'='*80}")
        print("EFFICIENT MULTI-SAMPLE ENTROPY vs QUESTION ACCURACY ANALYSIS")
        print(f"{'='*80}")
        ...
        if 'correlations' in analysis:
            ...
            for token_range, corr_data in analysis['correlations'].items():
                for measure in ['avg_entropy', 'max_entropy', 'entropy_variance']:
                    ...
                    if p_val < 0.1 and abs(corr) > 0.2:  # Relaxed thresholds for small sample
                        significant_findings.append(f"{measure} ({token_range}): r={corr:.3f}, p={p_val:.3f}")
            
            if significant_findings:
                print(f"\n🎯 NOTABLE FINDINGS (p < 0.1, |r| > 0.2):")
                for finding in significant_findings:
                    print(f"  ✓ {finding}")
            else:
                print(f"\n📈 No strong correlations found between entropy measures and question accuracy.")
